...
%  update: 11-02-18
% one unit with depression, different initial states
...
%% parameters
n_attractor = 1;  % index for initial attractor = {1,2}
...
ntrial = 300;
Iapp_vec = linspace(0,2,ntrial); 
% Iapp_vec = linspace(-2,2,ntrial); 
...
X0 = Xc_vec(:,attractors_idx(n_attractor))       % initial state 
...
data_filename = sprintf('phase_nc%d.txt', n_attractor);
dlmwrite(fullfile(data_pathname,data_filename),zm_colorcode,'delimiter','\t');
% 

% save figs
fig_pathname = './phase_diagram_figs/';
fig_filename = sprintf('phase_diagram_nc_%d', n_attractor);
...
data_filename4 = sprintf('initialState.txt');
% 
% data format
...
dlmwrite(fullfile(data_pathname,data_filename4),rc_attractors(:,n_attractor), ...
    '-append','delimiter','\t');
